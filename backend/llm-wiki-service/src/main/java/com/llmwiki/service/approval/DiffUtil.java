...
public class DiffUtil {
    ...
    public static String generateDiff(String before, String after) {
        ...
        for (AbstractDelta<String> delta : patch.getDeltas()) {
            switch (delta.getType()) {
                case INSERT -> sb.append("+ ").append(delta.getTarget().getLines()).append("\n");
                case DELETE -> sb.append("- ").append(delta.getSource().getLines()).append("\n");
                case CHANGE -> {
    ...
    public static String generateHtmlDiff(String before, String after) {
        ...
        for (AbstractDelta<String> delta : patch.getDeltas()) {
            switch (delta.getType()) {
                case INSERT -> delta.getTarget().getLines().forEach(
                        line -> sb.append("<ins>").append(escapeHtml(line)).append("</ins><br>"));
                case DELETE -> delta.getSource().getLines().forEach(
                        line -> sb.append("<del>").append(escapeHtml(line)).append("</del><br>"));
                case CHANGE -> {
                    delta.getSource().getLines().forEach(
                            line -> sb.append("<del>").append(escapeHtml(line)).append("</del><br>"));
                    delta.getTarget().getLines().forEach(
                            line -> sb.append("<ins>").append(escapeHtml(line)).append("</ins><br>"));
                }
            }
        }
